...
Sub Process_Globals
	...
	Type tipeumkm_lines (isi1 As String,isi2 As String,isi3 As String,isi4 As String)
...
Sub daftarjenis
	Spinner1.Add("--Choose Product Type--")
	ProgressDialogShow("Loading ....")
	ExecuteRemoteQuery("select * from umkm_type","KEC11") 'eksekusi query pencarian jenis ik
End Sub
...
Sub Button1_Click
	...
	ListView1.Clear
	ExecuteRemoteQuery("SELECT distinct a.id, a.name, a.address, a.cp, ST_X(ST_Centroid(a.geom)) AS longitude, ST_Y(ST_CENTROID(a.geom)) As latitude from umkm as a, umkm_type WHERE a.id_type=umkm_type.id_type and umkm_type.id_type='"&idkulkec&"' order by a.name asc","KECKUL11")
	WebView1.Visible=True
	WebView1.LoadUrl(""&Main.Server&"tipe_umkm.php?cari_nama="&idkulkec)
End Sub

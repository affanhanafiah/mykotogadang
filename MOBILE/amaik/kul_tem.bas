...
Sub Process_Globals
	...
	Private kul_tem = "kul_tem" As String
...
Sub iknama1
	...
	ExecuteRemoteQuery("SELECT distinct id, name,address, cp, st_x(st_centroid(geom)) as longitude, st_y(st_centroid(geom)) As latitude from culinary_place where lower(name) like lower('%"&inputnama11&"%')","kul_tem")
End Sub

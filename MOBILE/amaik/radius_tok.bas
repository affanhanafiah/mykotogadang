...
Sub Process_Globals
	...
	Type tok_lines (isi1 As String,isi2 As String,isi3 As String,isi4 As String, isi5 As String)
...
Sub mesjidradius
	...
	ExecuteRemoteQuery("SELECT id, name,st_x(st_centroid(geom)) as longitude,st_y(st_centroid(geom)) as latitude,st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), greatcharacter.geom) as jarak FROM greatcharacter where st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), greatcharacter.geom) <= "&radius1&" ","mesjid_radius")
	...
	If Job.Success Then
		...
		Select Job.JobName
			...
			If mes_rad_array.Size -1 < 0 Then
				Msgbox("Great Character Not Found", "Notice")
			Else
			Log("hy")
			ListView1.Clear
...
Sub Button1_Click
	...
	Else
		...
		ListView1.Clear
		WebView1.LoadUrl(""&Main.Server&"tok_radius.php?lat="&dashboardd.latUser&"&lng="&dashboardd.lngUser&"&rad="&radius1)
		mesjidradius
		End If
End Sub

...
Sub Process_Globals
	...
	Type jenisumkm_lines (isi1 As String,isi2 As String,isi3 As String,isi4 As String)
...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("umkm_product")
...
Sub daftarjenis
	Spinner1.Add("--Choose Product--")
	ProgressDialogShow("Loading ....")
	ExecuteRemoteQuery("select * from product","KEC11") 'eksekusi query pencarian jenis ik
End Sub
...
Sub Button1_Click
	...
	ListView1.Clear
	ExecuteRemoteQuery("SELECT distinct a.id, a.name, a.address, a.cp, ST_X(ST_Centroid(a.geom)) AS longitude, ST_Y(ST_CENTROID(a.geom)) As latitude from umkm as a, detail_product WHERE a.id=detail_product.id_umkm and detail_product.id_product='"&idkulkec&"' order by a.name asc","KECKUL11")
	WebView1.Visible=True
	WebView1.LoadUrl(""&Main.Server&"produk_umkm.php?cari_nama="&idkulkec)
End Sub

...
public class neartok extends Activity implements B4AActivity{
	public static neartok mostCurrent;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (isFirst) {
			processBA = new BA(this.getApplicationContext(), null, null, "b4a.example", "b4a.example.neartok");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (neartok).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "b4a.example", "b4a.example.neartok");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "b4a.example.neartok", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (neartok) Create, isFirst = " + isFirst + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (neartok) Resume **");
...
@Override
 ...
 protected void onStop() {
	...
	public void onPause() {
        ...
        BA.LogInfo("** Activity (neartok) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
...
public static String  _activity_create(boolean _firsttime) throws Exception{
 ...
 //BA.debugLineNum = 44;BA.debugLine="latTujuan=tokposisi.latTujuan";
mostCurrent._lattujuan = mostCurrent._tokposisi._lattujuan;
 //BA.debugLineNum = 45;BA.debugLine="lngTujuan=tokposisi.lngTujuan";
mostCurrent._lngtujuan = mostCurrent._tokposisi._lngtujuan;
...
_executeremotequery("SELECT id, name, st_x(st_centroid(geom)) As lng,st_y(st_centroid(geom)) As lat,st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), umkm.geom) as jarak FROM umkm where st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), umkm.geom) <= "+BA.NumberToString(_radius2)+"","umkm_nama4");
 };
...
_executeremotequery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), culinary.geom) as jarak FROM culinary where st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), culinary.geom) <= "+BA.NumberToString(_radius2)+"","umkm_nama4");
 };
...
_executeremotequery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), mosque.geom) as jarak FROM mosque where st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), mosque.geom) <= "+BA.NumberToString(_radius2)+"","umkm_nama4");
 };
...
_executeremotequery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), tourism.geom) as jarak FROM tourism where st_distance_sphere(ST_GeomFromText('POINT("+mostCurrent._lngtujuan+" "+mostCurrent._lattujuan+")',-1), tourism.geom) <= "+BA.NumberToString(_radius2)+"","umkm_nama4");
 };
...
mostCurrent._webview2.LoadUrl(""+mostCurrent._main._server+"sekitar.php?lat="+mostCurrent._lattujuan+"&lng="+mostCurrent._lngtujuan+"&kd1="+BA.NumberToString(_kd1)+"&kd2="+BA.NumberToString(_kd2)+"&kd3="+BA.NumberToString(_kd3)+"&kd4="+BA.NumberToString(_kd4)+"&kd5="+BA.NumberToString(_kd5));
 //BA.debugLineNum = 158;BA.debugLine="iknama4";
_iknama4();
...
_job._initialize(processBA,_jobname,neartok.getObject());
...
public static String  _iknama4() throws Exception{
 //BA.debugLineNum = 60;BA.debugLine="Sub iknama4";
...
switch (BA.switchObjectToInt(_job._jobname,_umkm_nama4)) {
case 0:

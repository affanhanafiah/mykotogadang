...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("detailinfotok")
	Log(id)
	If  tok_nama.id<>"" Then
		id_kul2=tok_nama.id		
		Log("nama umkm")
	...
	Else if kul_nama.id<>"" Then
			id_kul2=kul_nama.id
			Log("nama kul")	
	Else if radius_tok.id<>"" Then
			id_kul2=radius_tok.id
			Log("nama kul")			
...
Sub detailKUL2
	...
	ExecuteRemoteQuery("Select id, name, address, information, st_x(st_centroid(geom)), st_y(st_centroid(geom)) from greatcharacter where greatcharacter.id='"&id_kul2&"'","tok")
End Sub
...
Sub Button1_Click 
	...
	tokposisi.latTujuan=lat
	tokposisi.lngTujuan=lng	
	''activity menuju posisi industri kerajinan
	StartActivity("tokposisi")
End Sub

Sub Button2_Click 
	StartActivity(galeritok)
End Sub

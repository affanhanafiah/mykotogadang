...
Sub Process_Globals
	...
	Private rm_nama = "rm_nama" As String
...
Sub iknama1
	...
	ExecuteRemoteQuery("SELECT distinct id, name,address, cp,price, st_x(st_centroid(geom)) as longitude, st_y(st_centroid(geom)) As latitude from culinary where lower(name) like lower('%"&inputnama11&"%')","rm_nama")
End Sub
...
Sub JobDone(Job As HttpJob)
	...
	If Job.Success Then
		...
		Select Job.JobName
			Case rm_nama
				Dim rm_nama_array As List
				rm_nama_array = parser.NextArray
				ListView1.Clear
				For i=0 To rm_nama_array.Size -1
				Dim m As Map
				m = rm_nama_array.Get(i)
...
Sub EditText1_EnterPressed
	...
	Label2.Visible=True
	WebView1.LoadUrl(""&Main.Server&"rm_nama.php?cari_nama="&EditText1.Text)
End Sub
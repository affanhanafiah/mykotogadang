...
Sub Process_Globals
	...
	Type mosque_lines (isi1 As String,isi2 As String,isi3 As String,isi4 As String, isi5 As String)
...
Sub mesjidradius
	...
	ExecuteRemoteQuery("SELECT id, name,st_x(st_centroid(geom)) as longitude,st_y(st_centroid(geom)) as latitude,st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), mosque.geom) as jarak FROM mosque where st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), mosque.geom) <= "&radius1&" ","mesjid_radius")
	Log("SELECT id, name,st_x(st_centroid(geom)) as longitude,st_y(st_centroid(geom)) as latitude,st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), mosque.geom) as jarak FROM mosque where st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), mosque.geom) <= "&radius1&"")
End Sub
	...
	If Job.Success Then
		...
		Select Job.JobName
			...
			If mes_rad_array.Size -1 < 0 Then
				Msgbox("Mosque Not Found", "Notice")
			Else
			Log("hy")
			ListView1.Clear
...
Sub Button1_Click
	...
	Else
		...
		ListView1.Clear
		WebView1.LoadUrl(""&Main.Server&"mosque_radius.php?lat="&dashboardd.latUser&"&lng="&dashboardd.lngUser&"&rad="&radius1)
		mesjidradius
		End If
End Sub

...
Sub mesjidradius
	...
	ExecuteRemoteQuery("SELECT id, name,st_x(st_centroid(geom)) as longitude,st_y(st_centroid(geom)) as latitude,st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), umkm.geom) as jarak FROM umkm where st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), umkm.geom) <= "&radius1&" ","mesjid_radius")
	Log("SELECT id, name,st_x(st_centroid(geom)) as longitude,st_y(st_centroid(geom)) as latitude,st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), umkm.geom) as jarak FROM umkm where st_distance_sphere(ST_GeomFromText('POINT("&dashboardd.lngUser&" "&dashboardd.latUser&")', -1), umkm.geom) <= "&radius1&"")
End Sub
...
Sub Button1_Click
	...
	Else
		...
		ListView1.Clear
		WebView1.LoadUrl(""&Main.Server&"umkm_radius.php?lat="&dashboardd.latUser&"&lng="&dashboardd.lngUser&"&rad="&radius1)
		mesjidradius
		End If
End Sub

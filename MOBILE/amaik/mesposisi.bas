...
Sub posisi1 'menampilkan industri kerajinan pada yawebview
	WebView1.LoadUrl(""&Main.Server&"petaSayames.php?lat="&latTujuan&"&lng="&lngTujuan)
	Log(""&Main.Server&"petaSayames.php?lat="&latTujuan&"&lng="&lngTujuan)
End Sub
...
Log (id_ik2)
	...
	WebView2.LoadUrl(Main.Server&"petaSayames.php?lat="&latTujuan&"&lng="&lngTujuan&"&warna="&warna&"&id_angkot="&id_angkot&"&latsimpang="&latsimpang&"&lngsimpang="&lngsimpang)	
...
Sub Button2_Click 
	...
	ExecuteRemoteQuery("Select distinct angkot.id_angkot,angkot.destination, angkot.cost, angkot.track,detail_mosque.lat, detail_mosque.long, st_x(st_centroid(angkot.geom)) As longitude,st_y(st_centroid(angkot.geom)) As latitude FROM angkot left join detail_mosque on angkot.id_angkot=detail_mosque.id_angkot where detail_mosque.id_mosque='"&id_ik2&"'","ANGKOTIK")

...
Sub Activity_Create(FirstTime As Boolean)
	...
	latTujuan=kulposisi.latTujuan
	lngTujuan=kulposisi.lngTujuan
...
Sub Button1_Click 
	...
	kd1=1
	ExecuteRemoteQuery("SELECT id, name, st_x(st_centroid(geom)) As lng,st_y(st_centroid(geom)) As lat,st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), umkm.geom) as jarak FROM umkm where st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), umkm.geom) <= "&radius2&"","umkm_nama2")
	End If
	
	If CheckBox2.Checked Then
	kd2=1
	ExecuteRemoteQuery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), culinary.geom) as jarak FROM culinary where st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), culinary.geom) <= "&radius2&"","umkm_nama2")
	
	End If
	If CheckBox3.Checked Then
	kd3=1
	ExecuteRemoteQuery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), mosque.geom) as jarak FROM mosque where st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), mosque.geom) <= "&radius2&"","umkm_nama2")
	
	End If
	If CheckBox4.Checked Then
	kd4=1
	ExecuteRemoteQuery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), greatcharacter.geom) as jarak FROM greatcharacter where st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), greatcharacter.geom) <= "&radius2&"","umkm_nama2")
	
	End If
	If CheckBox5.Checked Then
	kd5=1
	ExecuteRemoteQuery("SELECT id, name, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), tourism.geom) as jarak FROM tourism where st_distance_sphere(ST_GeomFromText('POINT("&lngTujuan&" "&latTujuan&")',-1), tourism.geom) <= "&radius2&"","umkm_nama2")

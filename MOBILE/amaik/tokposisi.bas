...
Sub posisi1 'menampilkan industri kerajinan pada yawebview
	WebView1.LoadUrl(""&Main.Server&"petaSayatok.php?lat="&latTujuan&"&lng="&lngTujuan)
	Log(""&Main.Server&"petaSayatok.php?lat="&latTujuan&"&lng="&lngTujuan)
End Sub
...
ListView1.Clear
	...
	WebView2.LoadUrl(Main.Server&"petaSayatok.php?lat="&latTujuan&"&lng="&lngTujuan&"&warna="&warna&"&id_angkot="&id_angkot&"&latsimpang="&latsimpang&"&lngsimpang="&lngsimpang)	
...
Sub Button2_Click 
	...
	ExecuteRemoteQuery("Select distinct angkot.id_angkot,angkot.destination, angkot.cost, angkot.track,detail_greatcharacter.lat, detail_greatcharacter.long, st_x(st_centroid(angkot.geom)) As longitude,st_y(st_centroid(angkot.geom)) As latitude FROM angkot left join detail_greatcharacter on angkot.id_angkot=detail_greatcharacter.id_angkot where detail_greatcharacter.id_greatcharacter='"&id_ik2&"'","ANGKOTIK")

...
Sub Process_Globals
	...
	Private umkm_nama = "umkm_nama" As String
...
Sub iknama1
	...
	ExecuteRemoteQuery("SELECT distinct id, name,address, cp, st_x(st_centroid(geom)) as longitude, st_y(st_centroid(geom)) As latitude from umkm where lower(name) like lower('%"&inputnama11&"%')","umkm_nama")
End Sub
...
ListView1.Clear
	...
	Log(inputnama11)
	WebView1.LoadUrl(""&Main.Server&"umkm_nama.php?cari_nama="&EditText1.Text)
	iknama1
End Sub
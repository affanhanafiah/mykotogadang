...
var centerBaru;
  ...
  function init(){
    ...
    var url = 'http://192.168.1.2/kotogadang/andro/sekitar_rad_umkm.php?lat='+lat+'&lng='+lng+'&rad='+rad;
    console.log(url);
      $.ajax({url: url,url1, data: "", dataType: 'json', success: function(rows){ 
          for (var i in rows){ 
            var row = rows[i];
            var id = row.id;
